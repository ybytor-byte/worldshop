    ...
    for (const layer of layers) {
        try {
            ...
            if (layer.type === 'background') {
                map.setPaintProperty(layer.id, 'background-color', '#060612');
            }

            // — Water: deep dark ocean blue
            else if (layer.type === 'fill' && layer.id.includes('water')) {
                map.setPaintProperty(layer.id, 'fill-color', '#0c1a3d');
            }
            else if (layer.type === 'line' && layer.id.includes('water')) {
                map.setPaintProperty(layer.id, 'line-color', '#0e1f4a');
            }

            // — Land masses: visible dark teal/grey-green
            else if (layer.type === 'fill' && (layer.id.includes('land') || layer.id.includes('earth'))) {
                map.setPaintProperty(layer.id, 'fill-color', '#1a2a35');
            }

            // — Country fills (if any)
            else if (layer.type === 'fill' && layer.id.includes('country')) {
                map.setPaintProperty(layer.id, 'fill-color', '#1c2e3a');
            }

            // — Borders / boundaries: glowing purple-cyan lines
            else if (layer.type === 'line' && (layer.id.includes('border') || layer.id.includes('boundary'))) {
                map.setPaintProperty(layer.id, 'line-color', 'rgba(120, 160, 255, 0.45)');
                try {
                    map.setPaintProperty(layer.id, 'line-width', 1.2);
                } catch (e) {}
            }

            // — Country / city labels: bright enough to read
            else if (layer.type === 'symbol') {
                map.setPaintProperty(layer.id, 'text-color', 'rgba(220, 230, 255, 0.7)');
                try {
                    map.setPaintProperty(layer.id, 'text-halo-color', 'rgba(10, 10, 30, 0.9)');
            ...
            // — Roads: very subtle
                ...
                map.setPaintProperty(layer.id, 'line-color', 'rgba(80, 100, 140, 0.15)');
            }
