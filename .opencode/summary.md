...
- **MCP – `serper-shopping` URL fix**: controller now post‑processes Serper offers – maps `shop`/`source` to known Russian store domains (Ozon, Wildberries, Yandex Market, DNS, M.Video, Citilink, Eldorado, Regard, BigGeek, etc.), generates **direct search URLs** instead of Google Shopping redirects; unknown stores fall back to region‑appropriate marketplace (Ozon for RU, Amazon for US, etc.)
...
- **Google URLs → direct store URLs**: `serper_shopping` MCP tool post‑processes all offers – recognized stores get direct search URLs, unknown stores fall back to Ozon (RU) / Amazon (US) / MediaMarkt (EU)
